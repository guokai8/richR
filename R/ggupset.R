...
#'   single-set bars use the set color and multi-set intersection bars use a
#'   blend of the participating set colors.
...
ggupset <- function(x,
  ...
  bar_colors <- sapply(seq_len(n_intersects), function(i) {
    ...
    # Blend colors for multi-set intersections
  ...
  if (set.size.show) {
    ...
    # Align bar chart and matrix on shared intersection x-axis,
    # then position with ggdraw for pixel-accurate alignment
...
#' @param colors Character vector of color specifications
